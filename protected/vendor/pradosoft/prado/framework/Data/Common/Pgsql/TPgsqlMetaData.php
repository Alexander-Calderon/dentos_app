...
//		$command->bindValue(':columnIndex', join(', ', explode(' ', $columnIndex)));
	...
	{
		if ($schema === '') {
			$schema = $this->_defaultSchema; //self::DEFAULT_SCHEMA; se corrige esta porquería, entre otras..
		}
		$sql = <<<EOD
...
EOD;
		$command = $this->getDbConnection()->createCommand($sql);
		$command->bindParameter(':schema', $schema); // se cambia bindParam a bindParameter, ya que este no existe pero por defecto venía con errores.... -_- 
		$rows = $command->query(); // se modifica de command->queryAll(); a command->query(); ...no existía...
		$names = [];
		foreach ($rows as $row) {
